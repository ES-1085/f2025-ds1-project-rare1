...
Using an accessible color palette, we looked the GOLD assement scores for multi-year vs first time particiants of the Promise program.  We looked the weighted averageof the scores for each category, for First-Year participants, the sample size is 30 children, and for Multi-Year participants, the sample size is  51 children. We organized the bar graph with the catgories on the axis, and scores on the x-axis, and ordered them based on the highest weighted average score (from top down). The graph shows higher scores in each category for multi-year students, proving the effectivness of the Promise curriculum. 

```{r multi-year-plot, fig.alt = Bar Chart of GOLD Assessment Scores for Multi-Year Students: Compares the weighted average scores of First-Year, n=30 studnts, and Multi-Year, n=51 students, participants across six developmental categories. The chart shows that Multi-Year students achieved higher average scores in all six developmental areas, indicating greater mastery of developmental objectives.}
EHS_multi_year |>
  mutate(
...
```{r race-plot, fig.alt = Vertical Bar chart comparing the weighted average of the five categories(Physical, Cognitive, Language, Social-Emotional, and Mathematics) by race. Non-white scored higher then white in every category except language.There are 69 non-white students compared with 18 white students. Bar graph is color coded: non-white(purple) and white(yellow).}
EHS_with_race |>
